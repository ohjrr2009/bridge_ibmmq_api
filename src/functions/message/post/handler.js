...
module.exports.handler = async function (event, context) {
  ...
  const csfrToken = event.headers["ibm-mq-rest-csrf-token"]
  const mdCorrelationId =  event.headers["ibm-mq-md-correlationId"]
  const mdExpiry = event.headers["ibm-mq-md-expiry"]
  const mdPersistence = event.headers["ibm-mq-md-persistence"]
  const mdReplyTo = event.headers["ibm-mq-md-replyTo"]
  ...
  try {
    ...
    const msgId = await qmm.putMessage(body, csfrToken, mdCorrelationId, mdExpiry, mdPersistence, mdReplyTo)

    console.log(`Message sent successfully. ID returned: ${msgId}`)
    return httpResponse(201, '', context)
